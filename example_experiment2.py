...
# fmin = cma.fmin2
fmin = maes.maes
# fmin = maes.maes_midpoint
# fmin = maes.maes_midpoint_step
# fmin = maes.maes_midpoint_red
...
for batch_counter, problem in enumerate(suite):  # this loop may take hours or days...
    ...
    evalsleft = lambda: int(problem.dimension**2 * budget_multiplier + 1 -
    ...
    while evalsleft() > 0 and not problem.final_target_hit:
        ...
        if 11 < 3:  # add solver to investigate here
            ...
            fmin(problem, propose_x0(), lambda x: -problem.constraint(x), maxfun=evalsleft(),
                 disp=0, rhoend=1e-9)
        elif fmin is maes.maes:
            fmin(problem, problem.dimension, maxfun=evalsleft(), initial_pop=propose_x0())
        elif fmin is maes.maes_midpoint:
            fmin(problem, problem.dimension, maxfun=evalsleft(), initial_pop=propose_x0())
        elif fmin is maes.maes_midpoint_step:
            fmin(problem, problem.dimension, maxfun=evalsleft(), initial_pop=propose_x0())
        elif fmin is maes.maes_midpoint_red:
            fmin(problem, problem.dimension, maxfun=evalsleft(), initial_pop=propose_x0())
